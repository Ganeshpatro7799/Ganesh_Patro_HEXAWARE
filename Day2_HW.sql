...
insert into Users(UserName,Password,FullName,IsActive) values ('sam mc','5555','SAM MC',0);
 select* from ProductMaster
 insert into ProductMaster values('nails',500)
 insert into ProductMaster values('torch',1000);
 insert into ProductMaster values('switch',200);
 insert into ProductMaster values('TubeLight',20);
 insert into ProductMaster values('nails',500);
 
-------------------------------
alter table transactionn add check (TransactionType ='Inward');
insert into transactionn values ('2/20/2010', 4,'extra',40,'PETER')
alter table transactionn drop[CK__transacti__Trans__31EC6D26]
...
Update Users set UserID=5 where UserName='sam mc';

alter table transactionn add check (TransactionType in ('Inward','outward','extra'));

    insert into transactionn values ('2/16/2010', 7,'outward',50,'PETER');
    insert into transactionn values ('2/17/2010', 8,'outward',75,'JOHN MATHEW');
	insert into transactionn values ('2/18/2010', 5,'outward',25,'SAM MC');
    insert into transactionn values ('2/19/2010', 4,'outward',75,'JOHN MATHEW');
	-------------------------------
select * from transactionn
select * from transactionn where ProductID in(2,1);
select * from transactionn where Qty not in(5,10)
select * from transactionn where UpdatedBy like '%Ma%';
select TransactionDate from transactionn where UpdatedBY like '__M%';
select * from transactionn where UpdatedBY like '__M%';

--aggregate functions-----

select AVG(ProductId) as avg_PId from transactionn
select count (UpdatedBy) as row_count from transactionn
----(select ProductId ,count(Qty)as Try_ from transactionn)=ERROR- ---
select max(ProductId) from transactionn
------------------------------------------------------------
-----------------------------------------------------------------
-----------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------
...
select * from Region
-----------------------------------------------------------------------------------------1,2,9,10,11,13
--1.Write a query to display the id, entire street address and
--the first word of the street address of all entries in the location table.
--Give an alias to the first word as 'first_word'. Display the records sorted in ascending order based on first_word.

SELECT id, street_address, 
       SUBSTRING(street_address, 1, 1) AS first_word 
FROM location 
ORDER BY first_word ASC;
------and
select id,street_address,
SUBSTRING(street_address,1,CHARINDEX(' ',street_address)) as 'first_name'
from location order by first_name asc;
...
--Display the records sorted in ascending order based on state name.
 ...
 SELECT state AS State_Names from location
 where country_id='CA'
 ORDER BY State_Names ASC;
 ---------------and
 ...
 9-> select name from Countries where region_id=1 order by name asc;
 --------------------------------------------------------------------------------------------------------
 --	Write a query to display the id,entire street address and
 --the last word of the street address of all entries in the location table. Give an alias to the last word as 'last_word'.
 --Display the records sorted in ascending order based on last_word.
 select * from location;
select id,street_address,
Reverse(SUBSTRING(Reverse(street_address),1,CHARINDEX(' ',street_address))) as 'last_word' 
from location order by last_word asc;
-----------------------------------------------------------------------------------------------------------------------
...
---and
SELECT
  CONCAT('$', LPAD(salary, 10, ' ')) AS salary,
  first_name
FROM employee
...
select first_name,
SUBSTRING(phone_number,6,10) as 'phone_number' 
from employee 
order by first_name asc;
